 ...
 * +----------------------------------------------------------------------+
 * | +------------------------------------------------------------------+ |
 * | | `Serializer` - default formatter = `CompactFormatter`.
 * | +------------------------------------------------------------------+ |
 * +----------------------------------------------------------------------+
*/
 ...
 * +----------------------------------------------------------------------+
 * | +------------------------------------------------------------------+ |
 * | | `serde::ser::Serializer` for `Serializer`.
 * | +------------------------------------------------------------------+ |
 * +----------------------------------------------------------------------+
*/
  ...
  F: Formatter,
{
  ...
  fn serialize_struct(
    ...
    match name {
      ...
      crate::json::raw::TOKEN => Ok(Compound::RawDType { ser: self }),
...
pub enum Compound<'a, W: 'a, F: 'a> {
  ...
  #[cfg(feature = "raw_dtype")]
  RawDType { ser: &'a mut Serializer<W, F> },
}
  ...
  F: Formatter,
{
    ...
    T: ?Sized + Serialize,
  {
    match *self {
      ...
      #[cfg(feature = "raw_dtype")]
      Compound::RawDType { ref mut ser, .. } => {
        if key == crate::json::raw::TOKEN {
          tri!(value.serialize(RawDTypeStrEmitter(&mut *ser)));
          Ok(())
        } else {
          Err(invalid_raw_dtype())
        }
      }
    }
  }
 ...
 * +----------------------------------------------------------------------+
 * | +------------------------------------------------------------------+ |
 * | | `NumberStrEmitter` - feature = "arbitrary_precision"
 * | +------------------------------------------------------------------+ |
 * +----------------------------------------------------------------------+
*/
...
#[cfg(feature = "arbitrary_precision")]
impl<'a, W: io::Write, F: Formatter> ser::Serializer
  for NumberStrEmitter<'a, W, F>
{
  type Ok = ();
  type Error = Error;
 ...
 * +----------------------------------------------------------------------+
 * | +------------------------------------------------------------------+ |
 * | | `RawDTypeStrEmitter` - feature = "raw_dtype".
 * | +------------------------------------------------------------------+ |
 * +----------------------------------------------------------------------+
*/

#[cfg(feature = "raw_dtype")]
struct RawDTypeStrEmitter<'a, W: 'a + io::Write, F: 'a + Formatter>(
  &'a mut Serializer<W, F>,
);

#[cfg(feature = "raw_dtype")]
impl<'a, W: io::Write, F: Formatter> ser::Serializer
  for RawDTypeStrEmitter<'a, W, F>
{
  type Ok = ();
  type Error = Error;
  ...
  fn serialize_bool(self, _v: bool) -> Result<()> {
    Err(ser::Error::custom("expected RawDType"))
  }
  ...
  fn serialize_str(self, value: &str) -> Result<()> {
    let RawDTypeStrEmitter(serializer) = self;
    serializer
      .formatter
...
/// which allows the user to optionally pretty print the JSON output.
pub trait Formatter {
 ...
 * +----------------------------------------------------------------------+
 * | +------------------------------------------------------------------+ |
 * | | to_writer, to_writer_pretty - serialize to IO stream.
 * | +------------------------------------------------------------------+ |
 * +----------------------------------------------------------------------+
*/
 ...
 * +----------------------------------------------------------------------+
 * | +------------------------------------------------------------------+ |
 * | | to_vec, to_vec_pretty - serialize to byte vector.
 * | +------------------------------------------------------------------+ |
 * +----------------------------------------------------------------------+
*/
 ...
 * +----------------------------------------------------------------------+
 * | +------------------------------------------------------------------+ |
 * | | `to_string`, `to_string_pretty` - serialize to as string.
 * | +------------------------------------------------------------------+ |
 * +----------------------------------------------------------------------+
*/
