...
impl<'a> Read<'a> for StrRead<'a> {
  ...
  {
    let raw =
      &self.data[self.delegate.raw_buffering_start_index..self.delegate.index];
    visitor.visit_map(BorrowedRawDeserializer {
      raw_dtype: Some(raw),
    })
  }
 ...
 * +----------------------------------------------------------------------+
 * | +------------------------------------------------------------------+ |
 * | | Default implementations for `R` that implments `Read` trait.
 * | +------------------------------------------------------------------+ |
 * +----------------------------------------------------------------------+
*/
...
// Lookup table of bytes that must be escaped. A value of true at index
// `i` means that byte i requires an escape sequence in the input.
