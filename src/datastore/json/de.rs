...
where
  R: read::Read<'de>,
{
  /// Create a JSON deserializer from one of the possible `sage::json` input
  /// sources.
  ///
...
impl<'de, R: Read<'de>> Deserializer<R> {
  ...
  /// impls.
  ///
  /// *This method is only available if sage is built with the
  ...
  ///     }
  ///
  ///     let mut deserializer = sage::json::Deserializer::from_str(&json);
  ///     deserializer.disable_recursion_limit();
  ///     let deserializer = serde_stacker::Deserializer::new(&mut deserializer);
  ///     let value = DType::deserialize(deserializer).unwrap();
  ///
  ///     carefully_drop_nested_arrays(value);
  /// }
  ///
  /// fn carefully_drop_nested_arrays(value: DType) {
  ///     let mut stack = vec![value];
  ///     while let Some(value) = stack.pop() {
  ///         if let DType::Array(array) = value {
  ///             stack.extend(array);
  ///         }
  ///     }
  /// }
  ...
  fn parse_number(
    ...
    Ok(match tri!(self.peek_or_null()) {
      ...
      b'e' | b'E' => {
        ParserNumber::F64(tri!(self.parse_exponent(positive, significand, 0)))
      }
      _ => {
        if positive {
...
impl<'de, 'a, R: Read<'de>> de::Deserializer<'de> for &'a mut Deserializer<R> {
  ...
  ///
  /// The behavior of sage is specified to fail on non-UTF-8 strings
  ...
  ///     let bytes: ByteBuf = sage::json::from_slice(json_data)?;
  ///
...
impl<'de, 'a, R: Read<'de> + 'a> de::VariantAccess<'de>
  for VariantAccess<'a, R>
{
  type Error = Error;
...
impl<'de, 'a, R: Read<'de> + 'a> de::EnumAccess<'de>
...
///
///     let stream = Deserializer::from_str(data).into_iter::<DType>();
///
///     for value in stream {
...
where
  ...
  T: de::Deserialize<'de>,
{
  /// Create a JSON stream deserializer from one of the possible sage
  /// input sources.
  ///
  /// Typically it is more convenient to use one of these methods instead:
  ///
  ...
  /// let data = b"[0] [1] [";
  ///
  /// let de = sage::json::Deserializer::from_slice(data);
 ...
 * +----------------------------------------------------------------------+
 * | +------------------------------------------------------------------+ |
 * | | `from_*`: from_trait, from_reader, from_slice, from_str
 * | +------------------------------------------------------------------+ |
 * +----------------------------------------------------------------------+
*/
...
/// without being buffered in memory by sage.
///
/// When reading from a source against which short reads are not efficient, such
/// as a [`File`], you will want to apply your own buffering because sage
...
///     let u = sage::json::from_reader(reader)?;
///
///     // Return the `User`.
///     Ok(u)
/// }
...
///         }";
///
///     let u: User = sage::json::from_slice(j).unwrap();
///     println!("{:#?}", u);
/// }
/// ```
///
...
///         }";
///
///     let u: User = sage::json::from_str(j).unwrap();
///     println!("{:#?}", u);
/// }
/// ```
///
